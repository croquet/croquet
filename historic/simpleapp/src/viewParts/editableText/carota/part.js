import { nbsp, draw, measure } from "./text.js";
...
var prototype = {
  draw: function(ctx, x, y) {
    if (typeof this.run.text === 'string') {
      draw(ctx, this.run.text, this.run, x, y, this.width, this.ascent, this.descent);
...
export default function part(run, codes) {

  var m, isNewLine, code;
  if (typeof run.text === 'string') {
    isNewLine = (run.text.length === 1) && (run.text[0] === '\n');
    m = measure(isNewLine ? nbsp : run.text, run);
  } else {

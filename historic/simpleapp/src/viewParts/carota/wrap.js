...
export default function wrap(left, top, width, ordinal, parent,
  includeTerminator, initialAscent, initialDescent, margins) {
  var leading = margins && margins.leading || 0;
  var lineBuffer = [],
      lineWidth = 0,
  ...
  var send = function(emit) {
    ...
    var l = new Line(parent, left, width, y + maxAscent,
                     maxAscent, maxDescent, lineBuffer, ordinal);
    ordinal += l.length;
    quit = emit(l);
    y += (maxAscent + maxDescent) + leading;
    lineBuffer.length = 0;

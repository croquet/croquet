...
import { getPieceLength, getPiecePlainText } from './runs.js';
...
var section = function(runEmitter, codes) {
  ...
  s.parts.forEach(function(p) {
    ...
    s.width += p.width;
    s.length += getPieceLength(p.run.text);
    s.plainText += getPiecePlainText(p.run.text);
  });
  return s;
};

export default function word(coords, codes) {
  var text, space;
  if (!coords) {

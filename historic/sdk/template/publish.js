...
function buildGlobalNav(items, itemHeading, itemsSeen) {
    var nav = '';
    if (items.length) {
        var docdash = env && env.conf && env.conf.docdash || {};
        var globalNav = '';
        items.forEach(function(g) {
            if ( (docdash.typedefs || g.kind !== 'typedef') && !hasOwnProp.call(itemsSeen, g.longname) ) {
        ...
        if (!globalNav) {
            ...
            nav += '<h3>' + linkto('global', itemHeading) + '</h3>';
        }
        else {
            nav += '<h3>' + itemHeading + '</h3><ul>' + globalNav + '</ul>';
        }
    }
    return nav;
}
...
function buildNav(members) {
    ...
    var defaultOrder = [
        'Tutorials', 'Globals', 'Classes', 'Modules', 'Externals', 'Events', 'Namespaces', 'Mixins', 'Interfaces'
    ];
    ...
    var sections = {
        ...
        Globals: buildGlobalNav(members.globals, 'Globals', seen),
    };

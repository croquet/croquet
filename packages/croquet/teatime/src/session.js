...
export class Session {
     ...
     * @param {String?} parameters.step - `"auto"` (default) for automatic stepping via [requestAnimationFrame()](https://developer.mozilla.org/docs/Web/API/window/requestAnimationFrame), or `"manual"` to leave an application-defined main loop responsible for calling the session's `step()` function
     * @param {Number?} parameters.tps - ticks per second generated by reflector when no messages are sent by any user (`0` to `60`; default `20`)
     * @param {Number?} parameters.rejoinLimit - time in ms until view is destroyed after a disconnection, to allow for short network glitches to be smoothly passed over (default `1000`)

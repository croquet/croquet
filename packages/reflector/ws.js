...
function discover(ms, callback) {
    ...
    timeout = hotreloadEventManger.setTimeout(() => {
...
class CallbackHandler {
    ...
    _callback(event, ...args) {
        ...
        if (callback) {
            ...
            else hotreloadEventManger.promiseResolveThen(() => callback(...args));    // async but in order
        }
    }
}
...
hotreloadEventManger.addDisposeHandler("broadcast-channel", () => {
    if (channel) {
